...
import_memberships <- function(base = "app8BI6aTKIuB9U9y") {
  ...
  members <- members |>
    ...
    select(id, first_name, last_name, status = status_name, position_and_expertise_short, institution, country, website, twitter, github, photo_url, photo_filename, google, email) |>
    as_tibble() |>
  ...
  members_data <- members |>
    ...
    map(function(group) {
      group |>
        ...
        select(name, desc, img, website, twitter, github, google, email) |>
        mutate(across(c(website,google,email),
